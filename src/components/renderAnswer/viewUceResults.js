...
const ViewUceResults = () => {
  ...
  const renderSubQuestions = (subQuestions) => {
    return subQuestions.map((subQuestion, subIdx) => {
      const isScoreEqual = subQuestion?.user_score === subQuestion?.maxScore;
      ...
      const matchedCriteria = matchedAnswer
        ? matchedAnswer.response_criteria.find(
            (criteria) => criteria.response === subQuestion.user_answer
          )
        : null;
      ...
      return (
        <div key={subIdx}>
          ...
          >
            ...
            >
              {renderHTML(subQuestion?.user_answer || "No option selected")}
            </div>
            <div
              style={{
                display: "flex",
          ...
          {!isScoreEqual && (
            ...
            >
              ...
              >
                {renderHTML(subQuestion.correct_answer || "Not available")}
              </li>
            </ul>
          )}
        </div>
  ...
  const renderEitherOrQuestions = (category, startQuestion, endQuestion) => {
  ...
  return (
    <Container>
      ...
      <Card className="my-4">
        {/* Regular questions (1-3) for all subjects */}
        {(isBio || isPhy
          ? questionData.slice(0, 3)
          ...
          >
            ...
            {category?.questions?.sub_questions &&
              renderSubQuestions(category.questions.sub_questions)}
          </div>
        ))}

        {/* Either/Or questions for different subjects */}
        {(isBio || isPhy) &&
          questionData
            .slice(3, 4)
            .map((category, idx) =>
              renderEitherOrQuestions(category, "4", "5")
            )}
        ...
        {(isBio || isPhy) &&
          questionData
            .slice(4, 5)
            .map((category, idx) =>
              renderEitherOrQuestions(category, "6", "7")
            )}
        ...
        {(isMath || isGeo) &&
          questionData
            .slice(3, 4)
            .map((category, idx) =>
              renderEitherOrQuestions(category, "5", "6")
            )}
      </Card>
