...
import { uceFetchAndUpdateResults } from '../utilities/resultsUtil'
import {
...
function Login() {
      ...
      if (loginMethod === 'email') {
        ...
        if (userInfo.labels.includes('student')) {
          ...
          if (userInfo.educationLevel == 'PLE') {
            ...
            await uceFetchAndUpdateResults(session.userId)
          }
        ...
        if (userInfo.labels.includes('admin')) {
          try {
            ...
            console.error('Failed to retrieve transaction data', e)
          }
        }
  ...
  const handleLoginSuccess = async (userInfo) => {
    const unavailableEducationLevels = ['UACE'] // Education levels
  ...
  async function userData(accountId) {
    try {
      ...
      const response = await fetch(`${serverUrl}/get-user-details`, {
        method: 'POST',
        headers: {
              ...
              <Card className="shadow">
                <Card.Body>
                        ...
                        'Creation of a session is prohibited when a session is active.' ? (
                          <>
                            ...
                            <Button variant="primary" onClick={reFreshPage}>
                              Click to Refresh and Sign in again.
                            </Button>
                          </>
                        ) : (
                          <>
                            ...
                            <Button
                              ...
                              type="submit"
                              disabled={
                                emailLoginLoader || loginMethod === 'phone'
                              }
                            >
                              {emailLoginLoader ? (
                                <Spinner
                  ...
                  {loginMethod === 'email' && (
                    <>
                      <Row className="mt-4">
                        <Col className="text-center link-hover text-decoration-none">
