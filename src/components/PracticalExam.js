...
function PracticalExam() {
  const { userInfo, userPoints, userSubjectData, studentEnrollSubject } =
    useAuth();
  // console.log("user subjects: ", userSubjectData);
  // console.log("user enroll subjects: ", studentEnrollSubject);
  ...
  const subjects = (userSubjectData || []).filter(
  ...
  // added by ronnie  : to handle both ple and uce
  const handleStartExam = () => {
    ...
    console.log(selectedSubject.subjectType);

    if (selectedSubject.subjectType === "practical") {
      navigate(`/uce-exam/${subjectSlug}`);
    }

    if (levelSlug === "uce" && selectedSubject.subjectType === "theory") {
      // Navigate to /uce-exam/ and use only the first part of the subject name
      const subjectFirstName = selectedSubject.name.split(" ")[0].toLowerCase();
      navigate(`/uce-exam/${subjectFirstName}`);
    } else if (levelSlug === "ple") {
      // Keep the original behavior for PLE
      navigate(`/exam/${subjectSlug}_${levelSlug}`);
    }

    // console.log("Content is un-available");
  };
    ...
    return (
      <Card className="text-center" border="warning">
        ...
        <Card.Body>
          ...
          <Card.Text>
            It looks like your subscription has expired or you haven't
            subscribed yet. Subscribe now to regain access to all exams and
      ...
      <Container className="py-5">
        {userPoints < 1 ? (
          ...
          <>
              ...
              {isEnrolled ? (
                <>
                  {subjects.map(
                    (subject, index) =>
                      subject.enrolled &&
                      renderSubjectCard(subject, index, handleCardClick)
                  )}
                </>
              ) : (
                <Card>
                  <Card.Body>
                    <div className="text-center">
                      ...
                      <p>
                        Looks like you haven't enrolled for any subject. Ready
                        to challenge yourself? Enroll for a subject
                      </p>
                    </div>
                  </Card.Body>
                </Card>
            ...
            {enroll ? (
              <>
                <h3 className="text-center mb-4 subject-header">
                  Enrolling ...
                </h3>
                <Loader />
              </>
              ...
              <>
                ...
                <Row className="subject-row">
                  {subjects.map(
                    (subject, index) =>
                      !subject.enrolled &&
                      renderSubjectCard(subject, index, handleEnroll)
                  )}
                </Row>
              </>
            )}
            ...
            >
              ...
              {userPoints < 1 ? (
                <>
                  <Modal.Body>
                    <p>
                      "Oops! Your access has expired. Renew your subscription to
                      unlock this exam.
                    </p>
                  </Modal.Body>
                  <Modal.Footer>
                    ...
                    <Button
                      ...
                      style={{
                        backgroundColor:
                          selectedSubject?.color || "rgb(6, 63, 90) ",
                      }}
                    >
                      Subscribe to Exams
                    </Button>
            ...
            >
              ...
              <Modal.Footer>
                ...
                <Button
                  onClick={() =>
                    handleEnrollment(enrollSubject) && setEnrollSubject(null)
                  }
                  style={{
