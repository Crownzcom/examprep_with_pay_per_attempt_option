  ...
  try {
    ...
    if (refresh) {
      ...
      if (response.ok) {
        ...
        throw new Error(
          data.message || "Error fetching students data from the database"
        );
      }
    } else {
      ...
      if (response.ok) {
        ...
        console.warn(
          "Failed to fetch students datas from local file, trying the database refresh."
        );
...
export const fetchTransactions = async () => {
  try {
    ...
    if (response.ok) {
      ...
      throw new Error(
        data.message || "Error  transaction data from the database"
      );
    }
  } catch (error) {
...
async function updateStudentsLocalDatabase(studentData) {
  try {
    await db.transaction("rw", db.students, async () => {
      ...
      const savingToIndexDB = await db.students.bulkPut(
...
async function updateTransactionsLocalDatabase(transactionData) {
  try {
    await db.transaction("rw", db.transactions, async () => {
      ...
      // console.log('Saving to IndexDB ... ');
      // Bulk put the new data after clearing the table
      const savingToIndexDB = await db.transactions.bulkPut(
...
export const updateStudentDataInLocalStorage = async (studID, updatedData) => {
  try {
    ...
    const studentIndex = storedData.findIndex(
...
export const studentSubjectsData = async (
